...
def send_mail(missing_drivers, day_before_today, summ_of_transactions, num_of_transactions):
    ...
    try:
        ...
        log_file = open(os.path.abspath('www/bsk31.com/log/log.txt'), 'a+')
        # log_file = open(os.path.abspath('log/log.txt'), 'a+')
        log_file.write(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' загружено транзакций ' + str(num_of_transactions)  + ' на сумму ' + str(summ_of_transactions.quantize(Decimal("1.00"))) + ' письмо отправить не удалось. Код ошибки: ' + e + '\n')
        log_file.close()
